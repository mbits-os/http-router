...
namespace boost::beast::http::patched {
	...
	struct basic_file_body<file_win32> {
		...
		class value_type {
			...
			template <class Protocol,
			          ...
			          bool isRequest,
			          class Fields>
			friend std::size_t write_some(
			    net::basic_stream_socket<Protocol, Executor>& sock,
			    serializer<isRequest, basic_file_body<file_win32>, Fields>& sr,
			    error_code& ec);
		...
		public:
			...
			void reset(file_win32&& file, error_code& ec);
		};
		...
		public:
			...
			boost::optional<std::pair<const_buffers_type, bool>> get(
				...
				return {{{buf_, nread},         // buffer to return.
				         pos_ < body_.last_}};  // `true` if there are
				                                // more buffers.
			}
		};
		...
		public:
			...
			std::size_t put(ConstBufferSequence const& buffers,
				...
				for (auto buffer : beast::buffers_range_ref(buffers)) {
					nwritten +=
					    body_.file_.write(buffer.data(), buffer.size(), ec);
	...
	inline void basic_file_body<file_win32>::value_type::close() {
		error_code ignored;
		file_.close(ignored);
	}

	inline void basic_file_body<file_win32>::value_type::open(char const* path,
	                                                          file_mode mode,
	                                                          error_code& ec) {
	...
	inline void basic_file_body<file_win32>::value_type::reset(
	    file_win32&& file,
	    error_code& ec) {
		if (file_.is_open()) {
			error_code ignored;
	...
	namespace detail {
		...
		boost::winapi::DWORD_ lowPart(Unsigned n) {
			return static_cast<boost::winapi::DWORD_>(n & 0xffffffff);
		}

		template <class Unsigned>
		boost::winapi::DWORD_ highPart(Unsigned n, std::true_type) {
			return static_cast<boost::winapi::DWORD_>((n >> 32) & 0xffffffff);
		}

		template <class Unsigned>
		boost::winapi::DWORD_ highPart(Unsigned, std::false_type) {
			return 0;
		}

		template <class Unsigned>
		boost::winapi::DWORD_ highPart(Unsigned n) {
			return highPart(
			    n, std::integral_constant<bool, (sizeof(Unsigned) > 4)>{});
		}

		class null_lambda {
		public:
			template <class ConstBufferSequence>
			void operator()(error_code&, ConstBufferSequence const&) const {
				BOOST_ASSERT(false);
			}
		};
		...
		inline error_code make_win32_error(
			...
			return error_code(static_cast<int>(dwError), system_category());
		}

		inline error_code make_win32_error(error_code ec) noexcept {
			if (ec.category() != system_category()) return ec;
			return make_win32_error(
			    static_cast<boost::winapi::DWORD_>(ec.value()));
		}
		...
		class write_some_win32_op
			...
			net::basic_stream_socket<Protocol, Executor>& sock_;
			serializer<isRequest, basic_file_body<file_win32>, Fields>& sr_;
			bool header_ = false;
		...
		public:
			...
			write_some_win32_op(
			    ...
			    serializer<isRequest, basic_file_body<file_win32>, Fields>& sr)
			    : async_base<Handler, Executor>(std::forward<Handler_>(h),
			                                    s.get_executor())
			    , sock_(s)
			    , sr_(sr) {
				(*this)();
			...
			void operator()() {
				if (!sr_.is_header_done()) {
					header_ = true;
					sr_.split(true);
					return async_write_some_impl(sock_, sr_, std::move(*this));
				}
				if (sr_.get().chunked()) {
					return async_write_some_impl(sock_, sr_, std::move(*this));
				}
				auto& w = sr_.writer_impl();
				boost::winapi::DWORD_ const nNumberOfBytesToWrite = static_cast<
				    boost::winapi::DWORD_>(
				    (std::min<
				        std::uint64_t>)((std::min<
				                            std::uint64_t>)(w.body_.last_ -
				                                                w.pos_,
				                                            sr_.limit()),
				                        (std::numeric_limits<
				                            boost::winapi::DWORD_>::max)()));
				net::windows::overlapped_ptr overlapped{sock_.get_executor(),
				                                        std::move(*this)};
				// Note that we have moved *this, so we cannot access
				// the handler since it is now moved-from. We can still
				// access simple things like references and built-in
				// types.
				...
				auto const bSuccess = ::TransmitFile(
				    ...
				    nNumberOfBytesToWrite, 0, overlapped.get(), nullptr, 0);
				auto const dwError = boost::winapi::GetLastError();
				if (!bSuccess && dwError != boost::winapi::ERROR_IO_PENDING_) {
					// VFALCO This needs review, is 0 the right number?
					// completed immediately (with error?)
					overlapped.complete(make_win32_error(dwError), 0);
					return;
				}
				overlapped.release();
			}
		...
		struct run_write_some_win32_op {
			...
			void operator()(
			    WriteHandler&& h,
			    net::basic_stream_socket<Protocol, Executor>* s,
			    serializer<isRequest, basic_file_body<file_win32>, Fields>*
			        sr) {
				...
				static_assert(
				    beast::detail::is_invocable<
				        WriteHandler, void(error_code, std::size_t)>::value,
				    "WriteHandler type requirements not met");

				write_some_win32_op<Protocol, Executor, isRequest, Fields,
	...
	template <class Protocol, class Executor, bool isRequest, class Fields>
	std::size_t write_some(
		...
		if (!sr.is_header_done()) {
			sr.split(true);
			auto const bytes_transferred = write_some_impl(sock, sr, ec);
			if (ec) return bytes_transferred;
		...
		boost::winapi::DWORD_ const nNumberOfBytesToWrite = static_cast<
		    boost::winapi::DWORD_>(
		    (std::min<std::uint64_t>)((std::min<std::uint64_t>)(w.body_.last_ -
		                                                            w.pos_,
		                                                        sr.limit()),
		                              (std::numeric_limits<
		                                  boost::winapi::DWORD_>::max)()));
		auto const bSuccess =
		    ::TransmitFile(sock.native_handle(), w.body_.file_.native_handle(),
		                   nNumberOfBytesToWrite, 0, nullptr, nullptr, 0);
		if (!bSuccess) {
			ec = detail::make_win32_error(boost::winapi::GetLastError());
			return 0;
		}
	...
	async_write_some(
		...
		return net::async_initiate<WriteHandler, void(error_code, std::size_t)>(
		    detail::run_write_some_win32_op{}, handler, &sock, &sr);
	}
